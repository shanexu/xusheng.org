...
#+BEGIN_SRC scala -n
trait Component[T] { self: T =>
  @dom
  def render(implicit template: T => Binding[Node]) = {
    template(self).bind
  }
}
#+END_SRC
...
#+BEGIN_SRC scala -n
import org.scalajs.dom.raw.{ Node, Event, HTMLInputElement }
import com.thoughtworks.binding.dom
import com.thoughtworks.binding.Binding
import com.thoughtworks.binding.Binding.{ Var, Vars }

case class TagPicker(tags: Vars[String]) extends Component[TagPicker] {
  val text = Var("")

  val textChangeHandler = { e: Event =>
    e.target match {
  ...
  val addHandler = { e: Event =>
    text.get match {
      case "" | null =>
      case value if !tags.get.contains(value) =>
        tags.get += value
        text := ""
    }
  }

  val removeHandler = { value: String =>
    { e: Event =>
      tags.get -= value
    }
  }
...
object TagPicker {
  ...
  @dom
  implicit def defaultTemplate(self: TagPicker): Binding[Node] = {
    <section>
      <div>{
        for (tag <- self.tags) yield {
          <span>
            { tag }
            <button onclick={ self.removeHandler(tag) }>x</button>
          </span>
        }
      }</div>
      <div>
        <input value={ self.text.bind } onchange={ self.textChangeHandler }/>
        <button onclick={ self.addHandler }>Add</button>
      </div>
    </section>
  }
}
